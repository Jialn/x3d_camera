...
def convert_depth_to_color(depth_map_mm):
    depth_image_color_vis = depth_map_mm.copy()
    valid_points = np.where(depth_image_color_vis>=0.0001)
...
def saveX3dDepth(filepath, savepath):
    ...
    depth = depth[:,:,1]
    depth = convert_depth_to_color(depth*1000)
